...
set -uo pipefail
# Slurm on the login nodes defaults to configless DNS-SRV discovery, which fails
# here ("Unknown host"). The real config is on disk; point the client at it or
# every squeue/sacct/sbatch in this script dies before it runs.
